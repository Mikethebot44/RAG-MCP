...
export class ContentProcessor {
     ...
     */
    processDocumentationContent(content) {
        const chunks = [];
     ...
     */
    chunkGitHubFile(file, sourceUrl) {
     ...
     */
    chunkDocumentationPage(page, sourceUrl) {
        const baseChunkInfo = {
     ...
     */
    chunkCodeContent(content, baseInfo) {
        const chunks = [];
     ...
     */
    chunkByCodeStructure(content, baseInfo) {
        const chunks = [];
        const lines = content.split('\n');
     ...
     */
    addCodeChunk(chunks, lines, baseInfo, startLine, endLine) {
     ...
     */
    chunkTextContent(content, baseInfo) {
        const chunks = [];
        if (content.length <= this.maxChunkSize) {
            return [{
                    id: baseInfo.id,
     ...
     */
    chunkMarkdownContent(content, baseInfo) {
        const chunks = [];
        const lines = content.split('\n');
        let currentChunk = [];
        let currentHeading = '';
     ...
     */
    chunkByHeadings(content, headings, baseInfo) {
        ...
        for (const line of lines) {
            ...
            const matchedHeading = headings.find(heading => trimmedLine === heading ||
     ...
     */
    chunkByLines(content, lines, baseInfo) {
        const chunks = [];
        let currentLines = [];
        let chunkIndex = 0;
        for (const line of lines) {
     ...
     * Create a text chunk
     */
    createTextChunk(content, baseInfo, index) {
        return {
     ...
     */
    findTextBreakPoint(text, start, maxEnd) {
     ...
     * Determine file type
     */
    determineFileType(path, language) {
     ...
     */
    generateChunkId(sourceUrl, path, additional) {
